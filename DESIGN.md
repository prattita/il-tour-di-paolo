...
- [ ] Create Firebase project, enable Auth, Firestore, and Storage *(you — Firebase Console)*
- [x] Add Firebase config via environment variables (never commit to Git) — `.env.example`, `src/lib/firebase.js`
- [x] Write initial Firestore and Storage security rules — `firestore.rules`, `storage.rules`, `firebase.json` *(deploy: `firebase deploy --only firestore:rules,storage`)*
- [ ] Deploy app to Vercel and connect to GitHub (CI/CD) *(you — add `VITE_FIREBASE_*` in Vercel)*
- [x] `DESIGN.md` at repo root *(tracked in git; ensure not gitignored)*
