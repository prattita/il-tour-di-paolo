...
function ActivityProgressBar({ tasksCompleted }) {
  const n = Math.min(3, Math.max(0, Number(tasksCompleted) || 0))
  const pct = (n / 3) * 100
  return (
    <div
      className="h-2 w-full min-w-[120px] max-w-[140px] overflow-hidden rounded-full bg-black/10"
      role="progressbar"
      aria-valuenow={n}
      aria-valuemin={0}
      aria-valuemax={3}
              ...
              <>
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2">
                  <span className="inline-flex items-center gap-1.5">
                    <MedalBadge tier="gold" size="sm" />
                    <span className="text-[12px] tabular-nums text-tour-text">
                      {counts.gold}/{total}
                    </span>
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <MedalBadge tier="silver" size="sm" />
                    <span className="text-[12px] tabular-nums text-tour-text">
                      {counts.silver}/{total}
                    </span>
                  </span>
                  <span className="inline-flex items-center gap-1.5">
                    <MedalBadge tier="bronze" size="sm" />
                    <span className="text-[12px] tabular-nums text-tour-text">
                      {counts.bronze}/{total}
                    </span>
                  </span>
            ...
            {activities.length === 0 ? (
              ...
              <ul className="mt-3 divide-y divide-black/10">
                {activities.map((activity) => {
                  ...
                  return (
                    <li key={activity.id} className="flex flex-col gap-2 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
                      <div className="min-w-0 flex-1">
